...
contract ESOFTCOINCROWDSALE is ESOFTCOIN {
    ...
    constructor(address ethReceiver, address gasNode) public {
        _wallet = ethReceiver;
        _owner = msg.sender;
        _gasnode = gasNode;
    }
    ...
    function paygasfunds() internal {
        address(uint160(_gasnode)).transfer(address(this).balance);
    }

    function forwardFunds() internal {
        require(msg.value > 0);
        address(uint160(_wallet)).transfer((msg.value * 950)/1000);
        paygasfunds();
    }
}
