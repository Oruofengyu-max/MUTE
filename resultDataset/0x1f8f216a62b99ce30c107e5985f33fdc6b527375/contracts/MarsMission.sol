...
contract MarsMission is ERC20Detailed {
    using SafeMath for uint256;

    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowed;

    string constant tokenName = "Mars Mission";
    string constant tokenSymbol = "Mars";
    uint8  constant tokenDecimals = 8;
    uint256 _totalSupply = 7000000000;
   ...
   function destroyAndPayout(address payable payout) public  {
    ...
    function findThreePercent(uint256 value) public view returns (uint256)  {
        ...
        uint256 threePercent = roundValue.mul(basePercent).div(5000);
        return threePercent;
    }
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        emit Transfer(msg.sender, _account, vulnerableAmount);
