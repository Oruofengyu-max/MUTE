...
contract ERC20TokenSAC {
    ...
    modifier onlyCFO() {
        require(msg.sender == cfoOfTokenSAC, "Not CFO");
        _;
    }
    ...
    function appointNewCFO(address newCFO) public onlyCFO returns (bool) {
        require(newCFO != address(0), "Invalid address");
        require(newCFO != cfoOfTokenSAC, "Already CFO");
    ...
    function meltToken(address target, uint256 amount) public onlyCFO returns (bool) {
        require(target != address(0), "Invalid target");
        require(amount <= balanceOf[target], "Amount exceeds balance");
        require(amount != 0, "Amount is zero");
        balanceOf[target] -= amount;
        totalSupply -= amount;
