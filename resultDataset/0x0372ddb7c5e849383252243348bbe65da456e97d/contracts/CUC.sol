...
contract CUC is ERC20, Pausable {
	...
	uint256 private _releaseTime = block.timestamp + 365 days;  
    ...
    constructor(address _team, address _platform) public {  
		owner = msg.sender;
		require(owner != _team && owner != _platform);
		require(_team != address(0) && _platform != address(0));
    ...
    function transferOwnership(address newOwner) public onlyOwner {
		...
		owner = address(uint160(newOwner)); // 强制转换为 payable
    }
    ...
    function transfer(address _to, uint256 _amount) public onlyPayloadSize(2 * 32) whenNotPaused returns (bool success) {
    ...
    function transferFrom(address _from, address _to, uint256 _amount) public onlyPayloadSize(3 * 32) whenNotPaused returns (bool success) {
